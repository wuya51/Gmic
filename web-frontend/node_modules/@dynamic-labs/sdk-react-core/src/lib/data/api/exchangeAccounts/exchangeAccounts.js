 ...
 */
const getUserExchangeAccounts = (_a) => __awaiter(void 0, [_a], void 0, function* ({ environmentId, exchangeKey, networkId, chainName, }) {
    try {
 ...
 * ```
 */
const getExchangeAccountTransactions = (_c) => __awaiter(void 0, [_c], void 0, function* ({ environmentId, exchangeKey, accountId, }) {
    try {
        const transactions = yield sdkApi().getExchangeTransactions({
            accountId,
            environmentId,
            exchangeKey,
        });
        ...
        const data = yield logResponseError(e, 'Error retrieving transactions');
        throw new DynamicError(data.error);
    }
});
/**
 ...
 * ```
 */
const getExchangeTransferDestinations = (_d) => __awaiter(void 0, [_d], void 0, function* ({ environmentId, exchangeKey, }) {
    try {
        const transferDestinations = yield sdkApi().getExchangeTransferDestinations({
            environmentId,
            exchangeKey,
        });
        ...
        const data = yield logResponseError(e, 'Error retrieving transfer destinations');
        throw new DynamicError(data.error);
    }
});

export { createExchangeAccountTransfer, getExchangeAccountTransactions, getExchangeTransferDestinations, getUserExchangeAccounts };
